...
with tempfile.TemporaryDirectory() as tmp:
    ...
    psam.write_text('#IID\n' + '\n'.join(samples) + '\n')
    cov = p / 'cov.tsv'
    values = pd.DataFrame([rng.normal(size=n), genotypes[0] + rng.normal(scale=.1, size=n), genotypes[1] + rng.normal(scale=.1, size=n)], index=['GENETICPC1', 'PC1', 'PC2'], columns=samples)
    ...
    for chromosome, expected in [('chrY', {'v1', 'vX', 'vY_far'}), ('PC_SCAN', {'v1', 'vX', 'vY_near', 'vY_far'})]:
        ...
        subprocess.run([sys.executable, str(ROOT / 'tools/phenotype_pc_qtl/prepare_pc_scan.py'), '--covariates', str(cov), '--chromosome', chromosome], cwd=work, check=True)
        ...
        assert set(pairs.phenotype_id) == {'PC1', 'PC2'}
        assert len(pairs) == 2 * len(expected), pairs
        lead = pairs.loc[pairs.phenotype_id == 'PC1'].sort_values('pval').iloc[0]
