...
Merges per-sample PacBio 5mC BED calls after per-sample and cohort-level coverage QC. The input manifest is a TSV with `sample_id` and `file_path` columns; relative paths are resolved from the manifest's location. Each BED must contain `#chrom`, `begin`, `end`, and `cov` columns and must have the same schema as every other input BED.
...
- `--ValueColumn`: Optional numeric methylation column used for all-site methylation metrics and the site-by-sample matrix.
...
- `<OutputPrefix>.methylation.matrix.bed.gz` when `--ValueColumn` is supplied.
