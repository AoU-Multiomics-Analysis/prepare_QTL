...
- `scripts/methylation/FinalizeMethylationConnectivity.R` uses the correlation-pruned representative CpGs with the original full-matrix WGCNA biweight-midcorrelation connectivity calculation and removes failing samples consistently from final raw/INT BEDs and filtered calls.
- `scripts/methylation/MergeMethylationCohort.R` reduces one chromosome's shard outputs, derives passing calls from `per_sample_qc_pass`, applies cohort sample-presence and MAD filters, records an optional sample-normalized methylation–coverage correlation diagnostic (`--SkipCoverageMethylationCorrelation`), mean-imputes retained features, and writes chromosome-level raw and INT QTL phenotype BEDs. When skipped, the diagnostic metadata fields remain present with `NA` rho and zero contributing samples.
